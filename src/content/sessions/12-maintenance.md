...
Before Thursday, draft the eleven-row handover index and locate the actual rejected candidate, feedback/revision evidence and limitation within the existing independent-work block. Mark missing evidence as a gap. The final block includes [The rollback restores too much](#maintenance-audit-quiz): six new application cases, with scores and worked explanations revealed only after every case has a response and you submit.
...
3. **30 minutes — reproduce and roll back.** Use the first five minutes for [Release Day](/release-day/): carry in your Week 4 evidence note (or use the labelled supplied sample), trace one fault and save its repair/recheck. Then run the drill below against the unchanged Week 11 teaching example, or reproduce your own release using its own documented build. Preserve the received directory. Compare all **four declared output files**, exact candidate text, score and rank. Inspect the disposable faulty page, then the restored one. Record actual keyboard, focus, narrow-screen and text checks alongside the byte comparison. Explain why editing a manifest to match a fault, or restoring only HTML while leaving stale files, is insufficient.
4. **20 minutes — defend the limits.** Use ten minutes to inspect the actual Week 6–12 terminal checks and the Week 8 release-boundary result, and review your prepared eleven-row handover index. Check your own local paths, versions, results and concrete later uses against the actual rejected candidate, feedback/revision evidence and remaining limitation. Mark absent evidence as a gap; neither the example nor the template supplies student feedback or a completed submission. Spend about eight minutes answering [the six maintenance cases](#maintenance-audit-quiz), then two reviewing the unlocked explanations. Carry one correction into your manual or handover index.
...
The Release Day review trail can accompany this manual as exploratory practice. It preserves the received Week 4 note, your revised conclusion, record repairs and rechecks. Its fictional candidate is separate from the Week 11 reference; its green checks establish neither actual file integrity nor a completed student release. Keep the actual reproduction and browser evidence required below.
