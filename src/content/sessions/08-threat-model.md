...
Finish with [The flag factory requests a verdict button: the interactive quiz](#threat-audit-quiz). Six new cases include coded observations, detector tables and a population diagram. Apply the week's evidence and error-cost rules, then submit all six responses to unlock the score and worked explanations. The quiz is practice and carries no course marks.
...
4. **25 minutes — protocol review and quiz.** Use fifteen minutes to review your prepared implementation: six replaying the three review histories, six checking literal assertions and three exchanging a critique or writing a solo review. Put review around the existing Week 7 snapshot. Require all finding IDs and a note to clear a hold; retain incoming replies and observation expiry; preserve closed as terminal. Run the contact/payment release checks. Record a failing assertion from one deliberate mutation, restore it and verify the restored result. Use the final ten minutes for the [six-case quiz](#threat-audit-quiz): about eight minutes to respond and two to review the explanations after all cases are answered.
